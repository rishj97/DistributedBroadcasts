...
def start(peer, my_num, max_messages, timeout) do
    ...
    receive do
        {:broadcast, beb, peer_pl_list} ->
            ...
            broadcast(peer, peer_data, max_messages, cur_time() + timeout, my_num, beb)
    end
end

defp broadcast(peer, peer_data, broadcasts_left, end_time, my_num, beb) do
    cond do
        cur_time() >= end_time ->
            stop_broadcasting(peer, my_num, peer_data, peer)
        broadcasts_left > 0 ->
            receive do
                {:beb_deliver, peer_from, :broadcast_msg} ->
                    peer_data = receive_broadcast(peer_from, peer_data)
                    broadcast(peer, peer_data, broadcasts_left, end_time, my_num, beb)
            after
        ...
        true ->
            ...
            after
                ...
                0 -> broadcast(peer, peer_data, broadcasts_left, end_time, my_num, beb)
            end
    end
end

defp stop_broadcasting(peer, num, peer_data, peer) do
    # Display results
