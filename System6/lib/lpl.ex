    ...
    def start(beb, reliability) do
        ...
        receive do
            {:broadcast, peer_pl_list} ->
                ...
                start_listening(peer_pl_list, beb, reliability)
        end
    end

    defp start_listening(peer_pl_list, beb, reliability) do
        receive do
            {:pl_send, peer_from, peer_to, seq_no} ->
                {_, pl} = List.keyfind(peer_pl_list, peer_to, 0, nil)
                cond do
                    DAC.random(100) <= reliability ->
                        send pl, {:broadcast_msg, peer_from, seq_no}
                    true ->
                        nil
                end
